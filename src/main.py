...
def get_data():
    ...
    response = requests.request("GET", SIMPLY_PLURAL_BASE_URL+"/fronters/", headers=headers, data=payload, timeout=10)
    ...
    response = requests.request("GET", SIMPLY_PLURAL_BASE_URL + "/member/" + uid + "/" + fronters, headers=headers, data=payload, timeout=10)
    response_content = response.text
    data = json.loads(response_content)
